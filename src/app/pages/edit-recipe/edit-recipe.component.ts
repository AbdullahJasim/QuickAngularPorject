...
export class EditRecipeComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, private recipeService: RecipeService, private location: Location, private fb: FormBuilder, private router: Router) {
  ...
  private createForm(): void {
    ...
    this.recipe.instructions.forEach(item => {
      this.instructions.push(this.createInstruction(item));
    });

    this.recipe.ingredients.forEach(item => {
      this.ingredients.push(this.createIngredient(item.amount, item.name));
    });
  }
